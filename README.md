...
**Failure Reflection:**
- **First Divergence point:** The first time the robot's behavior deviated significantly from what we expected was in the second test case where we expected the robot to fail to look left when reliant on previous information (just saying look again after the original command), however, it we observed it succeeding at looking left. A similar deviance occurred in the third test case where we again thought the robot would fail, but it correctly looked left despite the d=2 distance between the orginal command and the follow-up.
- **Classify source of failure:** Since no failure occurred here, the classification instead applies to the earlier Section 3 failure of the follow up voice commands which would be classified as a sensing and perception issue because the robot doesn't understand what the users input is referring to. Possible explanations worth listing as inference could be that in section 3 the app wasn't restarted periodically and since Reachy had a lot of information at once, it's likely that it was harder for it to follow a context-based command. In our test cases the app was restarted for each condition making it easier for Reachy to follow up with our commands.
- **Evidence vs Inference:**
    - **Evidence:** In three controlled trials, with preceding app restarts, the "Look left" and "Look again" commands were correctly executed by Reachy as well as an appropriate response to our "How are you?" prompt in between. In the earlier Section 3 session, the same command failed after several prior turns without a restart.
    - **Inference:** We think the earlier failure was caused by the accumulated nature of the conversation (information overload), rather than a general inability to resolve follow up commands and are pretty confident in this interpretation because we saw through our test cases that Reachy is capable of previous information retrieval from the conversation, but there may be limits to its information capacity.
- **Migitations:**
...
- **Limitations & Follow up test:** Our simulation doesn't account for the lack of app restarts that we now hypothesize caused the failures in section 3, since it wasn't our independent variable and we did restart the app each time. A follow-up test that addresses this would be to run the boundary case multiple times with everything being the same, once immediately after a restart and again for various conversation lengths without restarting. This would directly test whether session length and information capacity are the actual causes of that failure.
